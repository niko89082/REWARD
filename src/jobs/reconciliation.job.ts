...
const reconciliationQueue = createQueue<ReconciliationJobData>('reconciliation');
 ...
 */
export async function processReconciliation(data: ReconciliationJobData): Promise<void> {
  const { merchantId, posIntegrationId, startDate, endDate } = data;

  try {
    logger.info(
      { merchantId, posIntegrationId, startDate, endDate },
...
export function startReconciliationWorker() {
  const worker = createWorker<ReconciliationJobData>(
    'reconciliation',
    async (job) => {
