 ...
 */
async function isQRTokenUnique(qrToken) {
 ...
 */
async function isPINUnique(pinCode) {
    const existing = await prisma.redemption.findUnique({
        where: { pinCode },
    });
    return !existing;
 ...
 */
export async function initiateRedemption(customerId, merchantId, rewardId) {
    try {
        // Get reward details
        ...
        logger.info({ redemptionId: redemption.id, customerId, merchantId, rewardId }, 'Redemption initiated');
        return redemption;
    }
    catch (error) {
 ...
 */
export async function verifyRedemption(code, merchantId) {
    try {
 ...
 * @param merchantId - Merchant ID
 */
export async function completeRedemption(redemptionId, merchantId) {
    try {
        await prisma.$transaction(async (tx) => {
            ...
            if (redemption.reward.type === 'POINTS_BASED' && redemption.reward.pointsCost) {
                await updateBalance(redemption.customerId, merchantId, -redemption.reward.pointsCost);
                // Record in ledger
                await recordRedeem(redemption.customerId, merchantId, redemption.reward.pointsCost, redemptionId);
            }
 ...
 */
export async function cancelRedemption(redemptionId, customerId) {
    try {
