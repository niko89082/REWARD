 ...
 */
export async function recordEarn(customerId, merchantId, points, transactionId) {
    try {
        ...
        logger.info({ entryId: entry.id, customerId, merchantId, points, transactionId }, 'Recorded points earned');
        return entry.id;
    }
    catch (error) {
 ...
 */
export async function recordRedeem(customerId, merchantId, points, redemptionId) {
    try {
        const entry = await prisma.ledgerEntry.create({
            data: {
                customerId,
        ...
        logger.info({ entryId: entry.id, customerId, merchantId, points, redemptionId }, 'Recorded points redeemed');
        return entry.id;
    }
    catch (error) {
 ...
 * @param transactionId - Transaction ID
 */
export async function recordRefund(customerId, merchantId, points, transactionId) {
    try {
        const entry = await prisma.ledgerEntry.create({
            data: {
                customerId,
        ...
        logger.info({ entryId: entry.id, customerId, merchantId, points, transactionId }, 'Recorded points refunded');
        return entry.id;
    }
    catch (error) {
 ...
 */
export async function getCustomerLedger(customerId, merchantId, limit = 50, offset = 0) {
    try {
 ...
 */
export async function recalculateBalance(customerId, merchantId) {
    try {
