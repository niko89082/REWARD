 ...
 */
function calculatePoints(amountCents, pointsPerDollar = 1) {
 ...
 */
export async function processWebhookEvent(provider, payload) {
    try {
 ...
 */
async function handlePaymentEvent(provider, event) {
    try {
        ...
        if (existing) {
            logger.info({ posTransactionId, provider, transactionId: existing.id }, 'Transaction already processed (idempotency)');
            return;
        }
        ...
        if (cardData) {
            ...
            customerId = await findOrCreateByCard(cardFingerprint, cardData.last_4, cardData.card_brand, provider);
        ...
        if (paymentData.itemizations || paymentData.line_items) {
            const lineItems = (paymentData.itemizations || paymentData.line_items || []).map((item) => ({
                id: item.uid || item.id,
            ...
            await processLineItems(customerId, integration.merchantId, lineItems);
        }
        logger.info({
 ...
 * Handle refund event
 */
async function handleRefundEvent(provider, event) {
    try {
        ...
        if (!originalTransaction) {
            logger.warn({ originalTransactionId, provider }, 'Original transaction not found for refund');
            return;
        }
        ...
        await updateBalance(originalTransaction.customerId, originalTransaction.merchantId, -refundPoints);
        await recordRefund(originalTransaction.customerId, originalTransaction.merchantId, refundPoints, originalTransaction.id);
 ...
 * Process line items for item-based rewards
 */
async function processLineItems(customerId, merchantId, lineItems) {
    if (!customerId) {
        return;
    }
    try {
        ...
        for (const lineItem of lineItems) {
            ...
            for (const reward of rewards) {
                ...
                const matches = (lineItem.catalogObjectId &&
